...
(function (Aurora) {
    ...
    Aurora.失心童话伤害 = {
        ...
        cast(skillData) {
            ...
            let atk = (0, Attack_1.genAttack)(skillData, 1, "雷电伤害", "所有伤害");
            for (let i = 0; i < 3; i++)
                rdt().getHit(atk);
            let count = user.getBuffStackCount(Aurora.噩廻);
            if (count >= 32) {
                let factor = 0.2 + (user.getBuffStackCount(Aurora.电棘丛生效果) * 0.2);
                let addatk = (0, Attack_1.genAttack)(skillData, factor, "雷电伤害", "所有伤害");
                rdt().getHit(addatk);
            }
        }
    };
    ...
    Aurora.荆雷奔袭 = {
        ...
        cast(skillData) {
            (0, Skill_1.procSTSkill)(skillData, (data) => {
                ...
                let atk = (0, Attack_1.genAttack)(skillData, 0.9, "雷电伤害", "所有伤害");
                for (let i = 0; i < 2; i++)
                    target.getHit(atk);
